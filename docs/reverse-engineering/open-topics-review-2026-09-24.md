...
| Text/pronunciation code labels | Resource framing, phone-ID expansion bytes, token/context layouts, and typed TPP grammar are documented. The 31,550-key corpus is exhaustively inventoried for numeric classes, component bits, and all 25 `AX` entries. A–D component-count/bit returns, literal `AX` return `X`, and F/G suffix writes to token byte `+0x25` are confirmed; F suffixes can be copied across a matched compound's component tokens. Static caller analysis plus a controlled marker/bit intervention confirms the B–E zero-bit rejection gate. A natural `N` → `North` replacement-table hit writes marker `d`; a matched `Main` lookup misses. The `A` marker appears in separate address-form branches. Same-input interventions show `G83` changing selected units and WAVs in selected questions, with matched pronoun/context controls and a phrase-specific `G74`/`G83` interaction. A scoped in-memory selector override retrieves both E records; disassembly shows the proper-name caller skips its selector switch for counts above four, and the other direct lookup callers use F/G. | **Runtime mechanics and tested acoustic effects are documented.** The corpus and probes do not supply authoritative meanings for all numeric classes or the linguistic rule assigning compound bits. An `AX` hit acts as a recognized-key marker in the tested single-name classifier path, but why these 25 entries use it remains unknown. The acoustic effect's feature and full context condition, possible indirect E callers, and broader per-key contexts remain open. See the [lead 3 TPP report](lead3-tpp-typed-code-findings-2026-09-24.md), [phone codebook](phone-symbol-codebook.md), and [Stage 10 contrasts](abbreviation-context-contrasts-2026-09-24.md). |
