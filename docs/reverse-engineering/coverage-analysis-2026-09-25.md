...
| File and buffer output APIs | Stage 16 checks `VT_TextToFile_ENG` selectors 0–10, selected errors, buffer formats 0–3 in the default mode, one repeated-call control, the format-0 synchronous `output_len` boundary, declared text-format values on ASCII, configuration clamps/output change, and selected playback returns. | Successful threaded processing, polling/completion, broader repeated-call state, successful playback, format-specific markup behavior, per-field configuration output effects, and other voices remain untested. See [Lead 6 results](lead6-file-api-behavior-2026-09-25.md). |
