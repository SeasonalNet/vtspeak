...
set logging file /work/stage7-followup4/postselection-gdb.log
...
commands
  silent
  set $backtrack_start = *(int *)($esp + 4)
  set $backtrack_end = *(int *)($esp + 8)
  set $state = *(unsigned int *)($esp + 12)
  set $ret = *(unsigned int *)$esp
  printf "BACKTRACK_ENTRY first_context=%d last_context=%d\n", $backtrack_start, $backtrack_end
  tbreak *$ret
  commands
    silent
    set $ctx = $backtrack_start
    while $ctx <= $backtrack_end
      set $record = $state + 0xae894 + $ctx * 0xfc
      set $index = *(short *)($state + 0xae0c4 + $ctx * 2)
      set $unit_count = *(unsigned short *)($record + 0xf4)
      set $unit = -1
      set $cost = 0.0
      if $index >= 0 && $index < $unit_count
        set $unit = *(unsigned int *)($record + 0x7c + $index * 4)
        set $slot = $ctx * 30 + $index
        set $cost = *(float *)($state + 0x8212c + $slot * 4)
      end
      printf "BACKTRACK_RESULT context=%d index=%d count=%u unit=%d cumulative_cost=%g\n", $ctx, $index, $unit_count, $unit, $cost
      set $ctx = $ctx + 1
    end
    continue
  end
...
commands
  ...
  if $segment_calls <= 41
    set $unit = *(int *)($esp + 8)
    printf "SELECTED_UNIT_HANDOFF call=%d unit=%d\n", $segment_calls, $unit
  end
  continue
end
