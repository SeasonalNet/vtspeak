set pagination off
set confirm off
set logging file /work/stage5/probes/stage6/records/word-encode-capture.log
...
commands
  silent
  set $word = *(unsigned int *)($esp + 4)
  set $encoded = *(unsigned int *)($esp + 8)
  set $ret = *(unsigned int *)$esp
  printf "WORD_ENCODE_ENTRY word=\n"
  x/s $word
  tbreak *$ret
  commands
    silent
    printf "WORD_ENCODE_RETURN bytes=\n"
    x/32bx $encoded
    continue
  end
  continue
