...
set $pcm_blocks = 0
set $selected_units = 0

break *0x1001b200
commands
  silent
  set $selected_units = $selected_units + 1
  printf "SELECTED_UNIT n=%u id=%u\n", $selected_units, *(unsigned int *)($esp + 8)
  continue
end
...
commands
  ...
  commands
    ...
    append binary memory /work/corpus-parity/stage11-current-blocks.pcm $pcm_out $pcm_out+$eax
    continue
  end
  continue
end
