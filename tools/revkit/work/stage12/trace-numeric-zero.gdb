...
commands
  ...
  if $code != 0
    printf "TPP_NUMERIC_ABLATE code=%u first=%u last=%u\n", $code, $first, $last
    ...
    commands
      ...
      while $i <= $last && $i < 64
        ...
        printf "TPP_NUMERIC_ZEROED_TOKEN index=%u code_byte_25=%u class_byte_26=%u surface=", $i, *(unsigned char *)($token + 0x25), *(unsigned char *)($token + 0x26)
        x/s ($token + 0x34)
        set $i = $i + 1
      end
      continue
